...
export const registerCollector = createAsyncThunk(
    ...
    async (userData, thunkAPI) => {
      try {
        const response = await authService.registerController(userData);
...
// Fetch the user profile data, including the totalAmountBySeller
...
const authSlice = createSlice({
  ...
  reducers: {
    ...
    setTotalAmountBySeller: (state, action) => {
      state.totalAmountBySeller = action.payload;
    },
  ...
  extraReducers: (builder) => {
    builder
      ...
      // Register Collector
      .addCase(registerCollector.pending, (state) => {
        state.isLoading = true;
      })
      .addCase(registerCollector.fulfilled, (state, action) => {
        state.isLoading = false;
        state.isSuccess = true;
        state.isLoggedIn = true;
        state.user = action.payload;
        toast.success("Collector registration Successful");
        console.log(action.payload);
      })
      .addCase(registerCollector.rejected, (state, action) => {
        state.isLoading = false;
        state.isError = true;
        state.message = action.payload;
        state.user = null;
